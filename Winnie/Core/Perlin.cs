...
{   
	...
	public class Perlin : IDisposable
	{
		...
		public Perlin(int seed, int sx, int sy, double min, double max, int st, int oc, double pers)
		{
		...
		[DllImport("libAlgo.dll", CallingConvention = CallingConvention.Cdecl)]
		extern static IntPtr Perlin_new(int seed, int sx, int sy, double min, double max, int st, int oc, double pers);
